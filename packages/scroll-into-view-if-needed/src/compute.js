...
const alignNearestBlock = (targetStart, targetSize, frame, frameRect) => {
    ...
    console.warn('top', 'target', targetStart, 'frame', frameRect.top, frameRect.width, frame.clientWidth, targetStart < frameRect.top);
    console.warn('bottom', 'target', targetEnd, 'frame', frameRect.bottom, targetEnd > frameRect.bottom);
    if ((targetStart < frameRect.top && targetSize < frame.clientHeight) ||
    ...
    if ((targetEnd > frameRect.bottom && targetSize < frame.clientHeight) ||
        (targetStart < frameRect.top && targetSize > frame.clientHeight)) {
        console.log('align to end', targetStart, frameRect.top);
        return targetStart - frameRect.top;
    }
    console.log('alignToNearest', 'targetStart', targetStart, 'should be 33 if clicking on 1', Math.min(targetStart, Math.max(targetEnd - frame.clientHeight, frame.scrollTop)));
    // start
...
// Find out the
const alignNearestInline = (targetStart, targetSize, frame, frameRect) => {
    // targetSize is either targetRect.height or targetRect.width depending on if it's `block` or `inline`
    const targetEnd = targetStart + targetSize;
    // Is not hidden by the starting edge (if block then this is typically frameRect.top)
    ...
    console.warn('left', 'target', targetStart, 'frame', frameRect.left, frameRect.width, frame.clientWidth, targetStart < frameRect.left);
    console.warn('right', 'target', targetEnd, 'frame', frameRect.right, targetEnd > frameRect.right);
    if ((targetStart < frameRect.left && targetSize < frame.clientWidth) ||
    ...
    if ((targetEnd > frameRect.right && targetSize < frame.clientWidth) ||
        (targetStart < frameRect.left && targetSize > frame.clientWidth)) {
        console.log('align to end', targetStart, frameRect.left);
        return targetStart - frameRect.left;
    }
    console.log('alignToNearest', 'targetStart', targetStart, 'should be 33 if clicking on 1', Math.min(targetStart, Math.max(targetEnd - frame.clientWidth, frame.scrollTop)));
    // start
...
export const compute = (maybeElement, options = {}) => {
    ...
    console.error('scrollMode', scrollMode, 'block', block, 'inline', inline, 'boundary', boundary, 'target', target);
    // Collect parents
    const frames = [];
    let parent;
    ...
    const computations = frames.map(frame => {
        ...
        if (block === 'start') {
            ...
            if (document.documentElement === frame) {
                ...
                const offset = Math.min(targetBlock - frameRect.top, frame.scrollHeight - frame.clientHeight - frame.scrollTop);
        ...
        if (block === 'center') {
            ...
            if (document.documentElement === frame) {
                ...
                const offset = 0 -
                    Math.min(frameRect.top + frameRect.height / 2 - targetBlock, frame.scrollTop);
                blockScroll = frame.scrollTop + offset;
        ...
        if (block === 'end') {
            ...
            if (document.documentElement === frame) {
                ...
                // prevent negative scrollTop values
                const offset = 0 - Math.min(frameRect.bottom - targetBlock, frame.scrollTop);
                blockScroll = frame.scrollTop + offset;
                // Cache the offset so that parent frames can scroll this into view correctly
                targetBlock += frame.scrollTop - blockScroll;
            }
        ...
        if (block === 'nearest') {
            ...
            if (document.documentElement === frame) {
                ...
                const offset = alignNearestBlock(targetBlock, targetRect.height, frame, frameRect);
                console.groupEnd();
                blockScroll = frame.scrollTop + offset;
                // Cache the offset so that parent frames can scroll this into view correctly
                targetBlock -= offset;
        ...
        if (inline === 'start') {
            ...
            if (document.documentElement === frame) {
                ...
                const offset = Math.min(targetInline - frameRect.left, frame.scrollLeft - frame.clientLeft - frame.scrollLeft);
        ...
        if (inline === 'center') {
            ...
            if (document.documentElement === frame) {
                ...
                const offset = 0 -
                    Math.min(frameRect.left + frameRect.width / 2 - targetInline, frame.scrollLeft);
                inlineScroll = frame.scrollLeft + offset;
                // Cache the offset so that parent frames can scroll this into view correctly
        ...
        if (inline === 'end') {
            ...
            if (document.documentElement === frame) {
                ...
                // prevent negative scrollLeft values
                const offset = 0 - Math.min(frameRect.bottom - targetInline, frame.scrollLeft);
                inlineScroll = frame.scrollLeft + offset;
                // Cache the offset so that parent frames can scroll this into view correctly
                targetInline += frame.scrollLeft - inlineScroll;
            }
        ...
        if (inline === 'nearest') {
            ...
            if (document.documentElement === frame) {
                ...
                const offset = alignNearestInline(targetInline, targetRect.width, frame, frameRect);
                console.groupEnd();
                inlineScroll = frame.scrollLeft + offset;
                // Cache the offset so that parent frames can scroll this into view correctly
                targetInline -= offset;
